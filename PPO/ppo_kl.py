...
# PPO with KL Penalty (PPO-Penalty)
# Based on Section 2 of the paper by Schulman et al. (2017)
...
class PPO_KL(object):
    def __init__(
        ...
        """
        Initialize PPO with KL-penalized surrogate objective
        ...
        Args:
            ...
            kl_target (float): Target KL divergence
            beta (float): KL penalty coefficient
            beta_update_factor (float): Multiplicative factor to update beta
        """
        ...
        self.beta = beta  # Adaptive KL penalty coefficient
    ...
    def train(self, buffer):
        ...
        for epoch in range(self.epochs):
            ...
            for start in range(0, states.shape[0], self.mini_batch_size):
                ...
                # Compute KL-penalized surrogate loss as described in Section 2 of PPO paper:
                # L^KLPEN(θ) = Ê_t[r_t(θ) * A_t - β * KL[π_θ_old, π_θ]]
                policy_loss = -(ratio * advantage_batch).mean() + self.beta * kl_div
        ...
        # Adaptive adjustment of the penalty coefficient beta
        # If KL > 1.5 * target_kl, increase beta
        # If KL < target_kl / 1.5, decrease beta
        if self.kl_div > self.kl_target * 1.5:
            self.beta *= self.beta_update_factor
        elif self.kl_div < self.kl_target / 1.5:
            self.beta /= self.beta_update_factor
